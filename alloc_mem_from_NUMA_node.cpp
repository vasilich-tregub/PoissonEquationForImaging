// https://learn.microsoft.com/en-us/windows/win32/memory/allocating-memory-from-a-numa-node
//
const bool VerifyNUMA = false;
...
int __cdecl wmain(int argc, const wchar_t* argv[])
{
    ...
    if (!VerifyNUMA) {
        _putts(_T("May be a fake allocation as the test `HighestNodeNumber == 0` is ignored\n"));
    }

    if (HighestNodeNumber == 0 && VerifyNUMA)
    {
